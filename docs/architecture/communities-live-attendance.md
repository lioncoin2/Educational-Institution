...
- **What exists today** means the repository at commit `9670c47`. Every such
  ...
  short path when the name is ambiguous).
...
Phase 0 and Live hardening (P1) change existing modules and are not gated.
...
| --- | --- | --- |
| Cross-module database or repository access | Each table is written and read only by its owner's infrastructure; ids cross modules as plain text; no cross-module foreign key | `no-cross-module-internals` (`.dependency-cruiser.cjs:141-161`); per-module schema-import and `pg_constraint` tests |
| Importing another module's infrastructure | Modules meet only at `contracts/` and `*.module.ts`; exports are contract tokens | the same rule; the new exports-are-contracts test (P0) |
| Business logic in controllers | One use case per act; controllers map DTOs and results | the existing API-layer rules |
| Business logic in Flutter screens | Screens render server capability booleans; controllers and repositories own flow | Flutter boundary tests (P0, P5, P7) |
| LiveKit SDK in domain code | Narrow RTC ports in `live/domain`; one adapter file | the fixed vendor-SDK rule, `livekit-sdk-only-in-the-live-adapter`, the rules-match spec (P0) |
| Messaging owning membership rules | Messaging asks `COMMUNITY_AUTHORIZATION` on every access; its participant rows for community chats are a named projection with no write port | `messaging-boundaries.spec.ts:27-41` (domain purity); 412 refusals on messaging's membership routes |
| Academic owning generic group infrastructure | The module is `communities`; no halaqa link in v1 ([Q50](open-questions.md#q50--communities-and-the-academic-structure)) | `communities-boundaries.spec` |
| Attendance reading LiveKit | Only `LIVE_PRESENCE`, allow-listed to attendance | `attendance-boundaries.spec` (P9) |
| A giant `GroupService`, `LiveService` or god service | One use case per act; `CommunityAuthorizationService` is one pure evaluator, not a façade | review; the exports-are-contracts test |
| Circular dependencies | A DAG with a topological order; three candidate back-edges rejected ([§3.4](#34-back-edges-and-the-rule-that-forbids-each)) | `no-circular` (`.dependency-cruiser.cjs:17-23`); the no-`forwardRef` test; per-module boundary specs |
...
| --- | --- | --- | --- |
| Community | A persistent space people belong to: the brief's Group | communities | Months or years; never deleted ([Q47](open-questions.md#q47--retiring-a-community)) |
...
| Capability grant | A delegated act, such as `community.live.moderate`, given by the owner to one stint | communities (P3) | Until revoked, or the stint ends |
...
| AttendanceSnapshot | One server-taken observation of whom the media provider held connected at a press | attendance (HELD) | Immutable ([Q71](open-questions.md#q71--correcting-retaining-and-erasing-snapshots)) |
...
| B. Messaging | one message, fanned out to online readers | Realtime: at most `1 + ⌈A/1000⌉` queries per message per instance, where A is the accounts connected to that instance (A ≤ 10,000, `realtime-policy.ts:42`). Notifications: one row per reader (Q28) | Fan-out tested at 250 members (`messaging-persistence.spec.ts:522-551`) |
| C. Live | one participant in one room on one node | The measured per-session cap (PROVISIONAL 300 plus a reserve of 10, [Q57](open-questions.md#q57--live-session-size-and-concurrency)) | Nothing |
...
| live (application) | communities | `COMMUNITY_AUTHORIZATION` (`community.live.start`, `.host`, `.moderate`, `.join`, `.raise_hand`); `COMMUNITY_MEMBERSHIP` (`heads` for effects, `statesOf` in batches of 1,000 for the reconciler); `COMMUNITY_CAPABILITY_HOLDERS` (moderators); subscribes to `communities.member.removed`, `communities.capability.revoked`, `communities.community.locked`/`unlocked` as accelerators | **new** |
...
attendance contracts. No `forwardRef` exists and a new test forbids it. No
token is provided outside the module that declares it.
...
| `COMMUNITY_AUTHORIZATION` | communities | new | P2 (grant basis P3) | messaging, live, attendance, communities' own use cases |
...
// Every act has three segments: it can never pass identity's CHECK, and isPermission(act) is false.
...
// participantsAmong = ACTIVE member (statesOf) with live.join (withPermission) and effects.liveJoinOpen, or a moderator.
// moderators = holders of community.live.moderate ∩ withPermission(live.moderate), plus the host while community.live.host holds.
...
live: { maxParticipantsPerSession /* PROVISIONAL 300, Q57 */; moderatorReserve /* PROVISIONAL 10, Q57 */; roomNamePrefix /* default 'live-' */ }
...
- ADR 0017 records the revision of Q1's provisional answer
...
([Q48](open-questions.md#q48--invitation-links)). Terms, races and refusals:
[communities.md](communities.md). Sequence:
[Appendix A1](#a1-join-through-an-invitation-link-then-open-the-chat).

## 10. Community lifecycle

OPEN ⇄ LOCKED; ARCHIVED is deferred
([Q47](open-questions.md#q47--retiring-a-community)); a community is never
deleted. What LOCKED means is one PROVISIONAL table owned by Communities
...
request; a grant comes only from a raised hand; the session's end expires
...
**HELD.** One provider read through `LIVE_PRESENCE` when a capability holder
presses Record; entries CONNECTED or CONNECTING under `provider_registry_v1`;
...
- **The outbox becomes mandatory** on any of four triggers: **T1** a
  ...
  sweeper, a reconciler and repair on access (T1 satisfied without an outbox).
...
| `communities.ownership.transferred` (P3) | communities | `{communityId, fromUserId, toUserId, transferredBy, basis, endedGrantIds (≤ 8)}` | communityId | R | realtime relay | `community.access.changed` to both users |
| `live.session.started` | live | `{sessionId, communityId, hostUserId}` | sessionId | R | realtime relay; notifications later (Q67) | frame to online accounts `LIVE_AUDIENCE.participantsAmong` accepts |
...
| `POST /communities {title}` | `communities.create` | re-authorized in the use case | 201; the creator becomes owner and first member. Not idempotent | 422 `communities.title_invalid`; 429 `communities.too_many_communities` |
...
| `POST /communities/:communityId/leave` | `communities.read` | the caller's own ACTIVE stint, not the owner | 204 | 404; 412 `communities.owner_cannot_leave` |
...
| `POST /communities/:communityId/grants {userId, capabilities[1..7]}` (P3) | `communities.moderate` | owner only; the owner's and grantee's ceilings | 201 `{created, unchanged}`; atomic batch | 404; 403 `communities.not_community_owner`; 403 `identity.permission_denied` (a missing ceiling); 422 `communities.grantee_ineligible` |
| `DELETE /communities/:communityId/grants/:grantId` (P3) | `communities.moderate` | owner only | 204, idempotent | 404 `communities.grant_not_found` |
| `PUT /communities/:communityId/owner {userId}` (P3) | `communities.read` | the owner, or oversight naming someone other than themself | 200; a repeat to the current owner is 200 | 409 `communities.owner_conflict`; 422 `communities.owner_ineligible`; 403 `communities.owner_self_assignment` |
...
| `POST /live/sessions/:sessionId/hand` (no body) | `live.raise_hand` | `community.live.raise_hand` | 201 new request; 200 the existing open request (today 409, `request-speaker.use-case.ts:71-75`) | 404; 412 `live.session_not_live`; 429 |
...
| Route | Declared | Use-case authorization | Success | Refusals |
| --- | --- | --- | --- | --- |
| `POST /attendance/live-sessions/:liveSessionId/snapshots {clientRequestId}` | authenticated | `LIVE_SESSIONS.describe` → `community.attendance.record` on its `communityId`; `recordedBy` = the principal; the whitelist rejects any other body field | 201 `SnapshotView` (counts only); 200 the stored snapshot for a replayed key | 400; 422 `attendance.client_request_id_invalid`; 404 `attendance.session_not_found`; 403 `attendance.not_allowed`; 412 `attendance.session_not_live`; 429 `attendance.too_many_snapshots`; **503 `attendance.observation_unavailable`, nothing stored** |
| `GET /attendance/communities/:communityId/snapshots?liveSessionId&cursor&limit` | authenticated | `community.attendance.view` | 200 headers, newest first (max 200); never calls Live | 404 `attendance.community_not_found`; 403 `attendance.not_allowed` |
| `GET /attendance/snapshots/:snapshotId` | authenticated | `community.attendance.view` on the header's community | 200 `SnapshotView` | 404 `attendance.snapshot_not_found` for unknown, invisible and forbidden alike |
| `GET /attendance/snapshots/:snapshotId/participants?connection&cursor&limit` | authenticated | as above | 200 `{items: [{userId, displayName, connection}], nextCursor}`; no "present" field | as above |
...
| live session started | `live.session.started` `{communityId, sessionId}` | online accounts that `LIVE_AUDIENCE.participantsAmong` accepts, probed in chunks of 1,000. The frame grants nothing | ≤ ⌈A/1000⌉ contract calls | the room exists | later (Q67) | |
...
| Controllers | `CommunityController`: applies `community.locked`/`unlocked` with a newer `lifecycleVersion` without a refetch; `member.removed` puts the community away; `access.changed` refetches. `LiveSessionController`: version-gated refetch; reconnect catch-up; `ROOM_DELETED` → refetch the session; `DUPLICATE_IDENTITY` → no automatic rejoin ([Q60](open-questions.md#q60--one-account-on-several-devices-in-a-session)) | P5, P7 |
| Deep links | `/invite`: reads the token from the link, holds it in memory, requires sign-in, POSTs it once in the body, replaces the history entry, never logs or prints it. Other links carry ids only and are resolved over HTTP | P5 |
...
| **Teacher device crashes** | The teacher rejoins from any device; LiveKit evicts the stale connection (`DUPLICATE_IDENTITY`). A presenter grant stays until stopped, revoked by another moderator (audited) or the session ends. If everyone leaves, the session ends as `idle` after 900 s observed empty (PROVISIONAL, Q61); LiveKit's own timeouts are a backstop only. |
| **Attendance request races a participant leaving** | The linearization point is the provider's registry read, inside `[observationStartedAt, observedAt]`: closed before it → absent; after it → included; inside LiveKit's short resume window after a transport failure → recorded as connected (the provider's semantics, recorded as is); mid-rejoin → CONNECTING. No retry, merge or grace. |
...
| Removed participant rejoins the room in a loop with refreshed tokens | Removed each time within 10–60 s; violations are counted and shown to moderators. The complete remedy is the media-room reset (P12, Q64). Residual: about 10–60 s of listening per cycle. |
...
| A load test aimed at production | Forbidden: a separate key pair and host ([Q65](open-questions.md#q65--media-hosting-and-operations)). |
...
| **Token reuse** | TTL 120 s; the reconciler's 60 s sweep and 10 s targeted watch; violations shown to moderators; the media-room reset (P12) invalidates every old token; `revokeTokensIssuedBefore` passed for providers that honour it; tokens never logged or put in events or frames | About 10–60 s of media per rejoin cycle for a determined holder until a moderator resets the room |
...
| Threat | Mitigation | Residual risk |
| --- | --- | --- |
| **Token refresh.** The server sends a connected participant a refreshed token every 5 minutes, valid for at least 10 minutes. The 120 s TTL bounds only the first connection. *Today the docs claim a leaked token is worth "ten minutes" (`realtime.md:505-510`); that is corrected.* | The reconciler is the enforcement, not the TTL: every connected identity is re-checked against the record and Communities each sweep | A removed participant's client holds a valid token for up to about 10 minutes |
| **`revoke_token_ts` is ignored** by the open-source server, so `removeParticipant` does not stop a rejoin | Targeted watch after every ejection, the participant sweep, violation counting; the epoch reset (P12) ends the loop because old tokens name a deleted room | As token reuse, above ([Q63](open-questions.md#q63--losing-standing-during-a-running-session)) |
...
| **`DUPLICATE_IDENTITY`**: a second connection with the same identity evicts the first | Identity = account id; the newest device wins; the app does not auto-rejoin on that reason (Q60); attendance keeps one entry per account | A stolen valid token can evict its owner for the token's life |
...
| The invitation token in deep links or logs | POST body only; the history entry is replaced; never logged or printed; the logger redacts the `token` body key | Static-host access logs may record the path; bounded by expiry and `maxUses` |
...
bounded by at most 4 speakers, 1 presenter, and listeners publishing nothing.
The reconciler costs, per live session per 60 s, one `listParticipants`
(unpaginated, proportional to P participants) plus ⌈P/1000⌉ membership and
directory probes. Session capacity is a deployment value copied onto each
...
- **SFU:** `lk load-test` (livekit-cli). Simulated publishers and subscribers
  ...
  documentation site could not be read here.)
...
| 4 | Large membership, no media | 30,000 and 100,000-member communities plus many small ones; a join storm through one link; community-chat fan-out; a lock with 10,000 sockets on one instance, 1 to 10,000 of them members | Keyset page latency (first, middle, last page); count; a 1,000-id probe; EXPLAIN plans; 1,000 concurrent joins against `maxUses` k; relay query counts and commit-to-last-send time; relay chain depth in a burst; the projection apply's lock hold time; realtime and notification cost per message |
...
with the evidence, and capacity configuration is set from the measured knees.
...
| Reconciler | With a fake provider seeded with participants: a member removed while connected is ejected within one tick even when no event was published; a divergent grant is re-applied; a revoked speaker still publishing loses the right; corrective acts audited with a null actor; a consistent session produces nothing; the regression where a speaker demoted 12 minutes earlier rejoins with a refreshed token | P6 |
...
| [0017](decisions/0017-community-scoped-authorization.md) | identity ceiling AND Communities standing AND the lifecycle gate; a closed three-segment act vocabulary; four bases; host-only moderation retired | Amends ADR 0005; revises Q1's provisional answer; supersedes the host-only paragraph of authorization.md |
...
| --- | --- | --- |
| [Q40](open-questions.md#q40--governance-which-gates-apply-to-the-new-modules) | Governance: which gates apply to the new modules? | Both the §13 step and the Attendance hold apply; only P0 and P1 proceed |
...
| [Q64](open-questions.md#q64--removing-a-participant-from-a-session) | Removing a participant from a session | Not built; seams only; media reset planned for P12 |
...
| **P9** Attendance (**HELD**) | The module; `LIVE_PRESENCE` and `listParticipants`; the attendance acts added by CHECK migration with ceilings using no `attendance.*`; `attendance-boundaries.spec`; Flutter `AttendanceRepository` | The user's ruling on Q40; reviewer acceptance of Q69; P6 | 20 same-key presses → 1 snapshot; churn tests equal the provider list; mid-read end → 412, nothing stored; attendance never imports LiveKit | **Q40, Q69**, P6 |
...
| **P12** Policy-gated Live features | Media-room reset by epoch; kick and re-entry; delegated or audio screen share; hidden listeners; webhook accelerators; `community.messages.moderate` | Q64, Q56, Q59, Q51/Q23 as each needs | Per feature | policy answers |
...
| 2 | Add `livekit-sdk-only-in-the-live-adapter` (modelled on the ws and push rules), and a live-boundaries spec asserting the adapter's edge exists | "LiveKit in exactly one file" is claimed but enforced only in domain and application; `communities/infrastructure` or `api` could import it undetected | `backend/.dependency-cruiser.cjs` (after `:117`); `backend/test/architecture/live-boundaries.spec.ts` (new) |
...
| 7 | Move Live's event names and payloads into `live/contracts/events.ts` as `LiveEvents`, byte-identical; factories import from `../contracts/events`; add `live.speaker.requested` to the events.md catalogue | Subscribers cannot import `live/domain` (type-only imports count); follows `messaging/domain/events.ts:1-9` | `backend/src/modules/live/domain/events.ts:9-77`; `backend/src/modules/live/contracts/index.ts:7`; `docs/architecture/events.md:174-197` |
...
| 11 | Correct docs and comments stating the retired or false design | Otherwise two modules appear to own attendance, and a new module could be misled about how permissions are added | `docs/architecture/module-boundaries.md:57-60`, `:146-149`, `:241-243`; `docs/architecture/realtime.md:573`; `docs/architecture/overview.md:146-148`; `backend/src/modules/operations/contracts/index.ts:1-6`; `backend/src/modules/live/domain/events.ts:3-7` |
...
| 13 | Governance: the user's ruling on Q40 before P2 and P9 start | A design must not lift a user constraint on its own | `docs/architecture/academic-reconciliation.md:19-21`, `:483-493` (read, not edited) |

### 25.2 Documents this package asks to change later

Not changed by this hub; each lands with the phase named.

| File | Change | Phase |
| --- | --- | --- |
| `module-boundaries.md` | Sections for communities and attendance (held); messaging depends on communities contracts and owns the link and projection; live is community-scoped with three new contracts; realtime depends on communities and live contracts; drop "operations derives attendance" (`:146-149`); fix `:57-60` | P0, P2, P6 |
| `open-questions.md` | Q1's provisional line revised when ADR 0017 lands; notes under Q20 and Q22 that they also cover communities; Q36 points to Q40 and Q50 | P0, P6 |
| `events.md` | Catalogue `live.speaker.requested`; add the communities, new live and attendance events; remove "operations (attendance)" from `live.session.ended`; durability classes and T1–T4 | P0, P2, P6 |
| `realtime.md` | Replace "no screen share by construction" (`:420-428`); correct the leaked-token claim (`:505-510`); mark the Redis queue and presence plan (`:542-553`) superseded; `:573` cites the new rule; add the matrix, frames, `OnlineAudience` and limits | P0, P5, P6 |
...
- **It did not carry out Phase 0.** Corrections 1–11 and 13 are proposals.
  Of item 12, only the numbering is taken: Q40–Q72 and ADRs 0016–0021
  (Proposed) are written as documents of this package.
- **It did not edit the existing documents in §25.2.** Their corrections land
  with the phases.
...
```
 App              Communities              Postgres (communities)     Bus              Messaging            Realtime
  ...
  │                    │──────────────────────────▶│                   │                   │                   │
  │                    │ 4 (P3) the link's creator still holds community.members.invite (Q48)
  │                    │ 5 UPDATE invitation SET uses+1 WHERE not revoked, not expired, uses < max_uses
  │                    │──────────────────────────▶│                   │                   │                   │
...
`communities.invitation_invalid` (an unknown token, or — from P3 — a creator
who lost the right to invite); 200 when already a member (step 3, no use
consumed, no audit, no event); 403 `communities.rejoin_requires_manager`; 412
`communities.invitation_revoked`, `invitation_expired`,
`invitation_exhausted` (step 5) or `community_locked` (step 6).
...
```
 Teacher app        Live                 Communities         Postgres (live)       LiveKit            Realtime          Member app
  ...
  │                    │────────────────────────────────────────────────────────────────────────────────▶│                   │
  │ 9 201 LiveSessionView                │                      │                     │                  │ 10 participantsAmong
  │◀───────────────────│                 │                      │                     │                  │    over online chunks
  │                    │                 │                      │                     │                  │──────────────────▶│
...
```
 Student app          Live                    Postgres (live)          LiveKit              Realtime            Moderator app
  ...
  │                    │ 3 S FOR SHARE (live); INSERT request ON CONFLICT (S,user) WHERE open DO NOTHING;             │
  │                    │   if inserted: state_version+1 → v                                                           │
  │                    │────────────────────────▶│                        │                    │                    │
...
```
 Teacher app        Attendance            Live (contracts)          Communities         LiveKit            Postgres (attendance)
  ...
  │                    │───────────────────────▶│                        │                   │                      │
  │                    │ 4 authorize(T, C, community.attendance.record) → not_found 404 / forbidden 403            │
  │                    │────────────────────────────────────────────────▶│                   │                      │
...
```
 Manager app     Communities        Postgres        Bus         Live (ProtectLiveSessions)   LiveKit        Messaging        Realtime     Removed app
  ...
  │               │                    │            │                   │    10–60 s and counts a violation; the media reset (P12) ends a loop   │
```
...
- If the process crashes after step 3, steps 4–12 never run. The participant
  ...
  projection within 60 s (PROVISIONAL, Q63).
