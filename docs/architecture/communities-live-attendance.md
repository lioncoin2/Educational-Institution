...
- **Everything else was a proposal when written.** P0–P5 and P5.1 have landed since
...
| **app** (Flutter data layer) | Extended in P5, P5.1 and P7; attendance in P9. P5 landed 2026-09-24: a read-only `CommunityRepository`, the `CommunityEvent` family, the capability models and the community screens. P5.1 landed 2026-09-26: the management calls (all but the direct add, deferred until a people-lookup policy exists), `me.operations` in the models, the management screens, and the `/invite` route on the web only ([§25](#25-implementation-phases)) | `CommunityRepository`, `LiveRepository` (HTTP and mock, bound only in `app_providers.dart`); `CommunityEvent` and `LiveEvent` frame families; the `LiveMediaClient` seam; server-parsed capability models; the `/invite` route | n/a | Screens never import `livekit_client`, `flutter_webrtc`, `web_socket`, `http`, `api_client` or repository implementations; never derive rights from roles; never label anyone "present" |
...
| `COMMUNITY_OPERATIONS` (in `capabilities.ts`: `community.invitations.manage`, `community.grants.manage`, `community.ownership.transfer`, `community.leave`) and `CommunityResponse.me.operations`: the one Communities contract addition P5.1 made, not in the approved catalogue. The operations that are not acts, each decided by the rule its route uses, so a client never works one out from standing or roles ([communities.md §6.6](communities.md#66-which-act-each-communities-operation-asks)) | communities | extend (additive; older apps ignore it) | P5.1 (landed) | Flutter (the management actions); no backend module |
...
| Repositories, frame families, `LiveMediaClient` | app | new | P5 (landed: the read-only `CommunityRepository`, `CommunityEvent`), P5.1 (landed: the repository's management calls, all but the direct add), P7, P9 | Flutter screens and controllers |
...
/** P5.1. What a caller may do that is not an act, reported in `me.operations` beside the acts so a client
 *  never works one out from standing or roles. Each is decided by the rule its route uses:
 *  LINK_MANAGEMENT_RULE (list and revoke links), decideOwnerOperation over MANAGE_GRANTS and
 *  TRANSFER_OWNERSHIP, and mayLeave. None is an act or a permission, and no grant gives one (tested). */
export const COMMUNITY_OPERATIONS = [
  'community.invitations.manage', 'community.grants.manage', 'community.ownership.transfer', 'community.leave',
] as const;
export type CommunityOperation = (typeof COMMUNITY_OPERATIONS)[number];
export function isCommunityOperation(v: string): v is CommunityOperation;
...
| `DELETE /communities/:communityId/members/:userId` | `communities.read` | `community.members.remove` (allowed while LOCKED; P3 subset rule) | 204 | 404 `communities.member_not_found`; 412 `communities.owner_not_removable`; 422 `communities.cannot_remove_self` (leaving is `/leave`); 403 `communities.member_holds_more_capabilities` (the subset rule); 403 |
...
| `POST /communities/:communityId/grants {userId, capabilities[1..7]}` (P3) | `communities.moderate` | owner only (Q44); the owner's and grantee's ceilings | 201 `{created, unchanged}` when any grant was created, 200 when all were already held; atomic batch | 404; 403 `communities.not_community_owner`; 403 `identity.permission_denied` (a missing ceiling); 422 `communities.grantee_ineligible` |
| `DELETE /communities/:communityId/grants/:grantId` (P3) | `communities.moderate` | owner only (Q44) | 204, idempotent | 404 `communities.grant_not_found`; 403 `communities.not_community_owner` |
| `PUT /communities/:communityId/owner {userId}` (P3) | `communities.read` | the owner, or oversight naming someone other than themself (Q42) | 200; a repeat to the current owner is 200 | 409 `communities.owner_conflict`; 422 `communities.owner_ineligible`; 403 `communities.owner_self_assignment`; 403 `communities.not_community_owner` (a member who is not the owner) |

`CommunityResponse` is `{id, title, status, lifecycleVersion, memberCount,
createdAt, me: {standing, joinedAt, capabilities, participation, operations}}`.
`operations` was added in P5.1, additively: the operations that are not acts
(`community.invitations.manage`, `community.grants.manage`,
`community.ownership.transfer`, `community.leave`), each listed exactly when
its route would allow it now
([communities.md §12](communities.md#12-api)). The `me`
...
- **Reconnect:** on `reconnected` the client refetches the open community and
  ...
  read again whenever the connection comes up, connected or reconnected; since
  P5.1, so do an open list of invitation links and a member's grants
...
`/communities/:communityId/members`, reached from Profile. P5.1 (landed
2026-09-26) added the management rows marked below, and two routes:
`/communities/:communityId/invitations`, reached from the community screen,
and `/invite`, where an invitation link opens (`app/lib/app/routes.dart:21`,
`:42-43`).

| Piece | Proposal | Phase |
| --- | --- | --- |
| `CommunityRepository` | `communities({scope, cursor})`, `community(id)`, `members(id, cursor)`; `addMembers`, `removeMember`, `leave`; `createInvitation(id, terms)` → `{invitation, token}` (token shown once); `invitations`, `revokeInvitation`; `join(token)` (POST body); `lock`, `unlock`; `grants`, `grant`, `revokeGrant`, `transferOwnership` (P3). HTTP + mock. **As landed (P5): reads only** — `communities({cursor})` (always `scope=mine`), `community(id)`, `members(id, {cursor})`, HTTP and mock. **As landed (P5.1):** `createInvitation(id)` sends `{}`, no terms, so the server's defaults apply (Q48); `invitations(id, {cursor})`, `revokeInvitation`; `join(token)`, the token in the body only; `removeMember`, `leave`; `lock`, `unlock`; `grants(id, {userId, cursor})`, `grant(id, {userId, capabilities})`, `revokeGrant`; `transferOwnership` (`PUT …/owner`, through the new `ApiClient.put`). HTTP and mock. Each write is one request, never retried by the repository; a refusal keeps the server's `details` (`repositories.dart:204-289`, `http_community_repository.dart`). There is **no `addMembers`**: the direct add is deferred until a people-lookup policy exists (Q6, Q22, Q31, Q50) | P5: reads landed; P5.1: the rest landed but `addMembers`, deferred |
...
| Capability and state models | `CommunityView {id, title, state (+unknown), lifecycleVersion, memberCount, me: {standing, capabilities: Set<CommunityCapability (+unknown)>, participation}}`; `LiveSessionView` with `stateVersion` and server-computed `me` flags. Unknown enum values map to `unknown` and are ignored; missing booleans are false. **As landed (P5):** `Community {id, title, status (+unknown), lifecycleVersion, memberCount, createdAt, me: CommunityMe {standing (null: oversight), joinedAt, capabilities, participation}}` in `lib/data/models/communities.dart`, with `canOpenChat` (participation holds `community.chat.read`) and `canViewMembers` (capabilities hold `community.members.view`). **As landed (P5.1):** `CommunityMe.operations`, a `Set<CommunityOperation>` read like the capabilities (an unknown value is dropped, so it unlocks nothing; missing from an older server's answer, nothing is allowed), with `allows(operation)`; `CommunityInvitation` with `InvitationState` (+unknown), shown as the server states it (ACTIVE promises no join); `CreatedInvitation`, the one answer that carries a token, whose `toString` leaves it out; `CommunityGrant` (with `dormant`), `GrantChange`; `CommunityException.details` (the server's `retryAfterSeconds`, `permission` or `field`), never shown as is | P5 (landed), P5.1 (landed), P7 |
...
| Controllers | `CommunityController`: on `community.locked`/`unlocked` with a newer `lifecycleVersion`, records the version and makes one single-flight refetch of `GET /communities/:id`, because `me.capabilities` includes the lifecycle gate and is never recomputed on the client; `member.removed` puts the community away; `access.changed` refetches. `LiveSessionController`: version-gated refetch; reconnect catch-up; `ROOM_DELETED` → refetch the session; `DUPLICATE_IDENTITY` → no automatic rejoin ([Q60](open-questions.md#q60--one-account-on-several-devices-in-a-session)). **As landed (P5):** `CommunityController` as above, and the viewer's `member.added`, or the connection coming up, also refetches; `CommunityListController`, `CommunityMembersController`, and the chat opener, which asks `GET /messaging/communities/:communityId/conversation` and opens the ordinary conversation route; messaging's controllers follow community frames ([realtime.md §C6](realtime.md#c6-the-clients-contract-for-community-frames)). **As landed (P5.1):** the viewer's own changes — `CommunityController.lock`, `unlock`, `leave`; `CommunityMembersController.remove`, `transferOwnership`; `CommunityInvitationsController` (links a page at a time; `create`, `revoke`); `MemberGrantsController` (one member's grants; grant, revoke); `InvitationJoinController` (the invite screen) — each one request per target, a second tap while it is on its way sending nothing, and never retried on its own (a new link is not idempotent). Nothing is shown changed before the server answers; then each controller concerned reads again through its own one-at-a-time read, and a read sent before a confirmed change never overwrites what came after it. A refusal that answers for the community reads it again too, so an action whose right is gone disappears ([realtime.md §C6](realtime.md#c6-the-clients-contract-for-community-frames)) | P5 (landed), P5.1 (landed), P7 |
| Deep links | `/invite#<token>`: the token travels only in the URL fragment, which browsers never send in requests or `Referer` headers (the web app uses the path URL strategy, `url_strategy_web.dart:9`, so a path or query would reach the static host). The app reads it from the fragment, holds it in memory, requires sign-in, POSTs it once in the body, replaces the history entry, never logs or prints it. Other links carry ids only and are resolved over HTTP. **As landed (P5.1), on the web only** (the user's decision, 2026-09-26): `main()` takes the token from the address before `runApp` (`app/lib/main.dart:12-26`), because building the router makes the web engine rewrite the address without its fragment; it rubs the token out of the address bar at once with `replaceState`, keeping the history entry's state (`app/lib/app/invite_link_parts.dart`, `invite_link_web.dart`); and a popstate listener registered before the engine's catches a link opened while already on `/invite`, which changes only the fragment. The token is then held in memory in one place (`pendingInvitationProvider`, `features/communities/state/pending_invitation.dart`), never in a route, a query, `extra`, storage, a log or a `toString`. Joining asks first: the invite screen says only that the viewer has been invited, and one Join tap sends one POST; a token without the contract's shape sends nothing. The web app writes a link as `<its base href>invite#<token>`; native builds read none and write none (`invite_link_stub.dart`), and no mobile app link is configured | P5 in the plan; **landed in P5.1, on the web only**; mobile app links deferred |
| Mock parity | `MockCommunityRepository` and `MockLiveRepository` reproduce the server rules (idempotent join, max-uses refusal, locked refusals per the PROVISIONAL table (Q46), one open hand, the speaker cap, idempotent end) and carry `DataOrigin.mock`. Mock mode never produces a usable media grant. **As landed (P5):** `MockCommunityRepository` serves the reads — refusals for an unknown community and for a roster without `community.members.view`, the lifecycle applied to `me`, and a 30,000-member community whose roster is built a page at a time; join, max-uses and invitation parity wait for the deferred management calls. **As landed (P5.1):** every management call, refused rule by rule as the server refuses it, each copied rule labelled "PROVISIONAL, Qnn — the server's table" (Q42, Q44, Q45, Q46, Q48, Q49); links on the server's default terms, their token shown once, their state derived like the server's; joining idempotent for a member, and refused for a link expired, revoked or used up, for a removed member, and while LOCKED; `me.operations` decided by the same predicates as the mock's routes; the 30,000-member roster still built a page at a time after removals. The mock knows no roles and has no oversight, so its transfer target's eligibility is its own stated fiction. One community the viewer is not in, `mock-community-dawn`, carries links with fixed demo tokens, so the demo can open one (`MockCommunityRepository.demoActiveToken` and the refused ones; `app/README.md`), and the demo's messaging reads and posts in a community's chat as that community's `me` says now | P5 (reads landed), P5.1 (landed), P7 |
| Guards | No `livekit_client`, `flutter_webrtc` or `dart_webrtc` in `pubspec.yaml` or `lib/` (P0; after P7b, one adapter file only). Screens and `core/widgets` never import `package:http`, `web_socket`, `api_client.dart`, repository implementations or `websocket_realtime_client.dart`. Community and live features never read `CurrentUser.permissions` or roles. Nothing labels anyone "present" or turns a snapshot into a ratio. **As landed:** `live_boundaries_test.dart` (P0); `community_boundaries_test.dart` (P5) for community screens, widgets and state. P5.1 turns its read-only check into the exact list of calls the HTTP repository may make (a new call is a deliberate change to the list) with no POST to `…/members`, keeps web-only code in `lib/app` (a feature writes a link only through `inviteLinkBuilderProvider`), forbids logging in every file a token passes through, and checks that neither `PendingInvitation` nor `CreatedInvitation` prints its token | P0 (landed), P5 (landed), P5.1 (landed), P7, P9 |
...
| **Unauthorized community access** | Every route authorizes server-side: ceiling, then the stored stint, grant or oversight, then the gate. `me.capabilities` is display only, as is `me.operations` (P5.1). The principal is rebuilt from storage on every request | None known for the acts designed |
...
| The invitation token in deep links or logs | In the link, only in the URL fragment (`/invite#<token>`), which is never sent to a server; to the API, only in the POST body; the history entry is replaced; never logged or printed; the logger redacts the `token` body key. As landed (P5.1, web only): read in `main()` before the router exists and rubbed out of the address bar at once (the entry keeps its state), then held in memory in one holder; `community_boundaries_test.dart` forbids logging wherever it passes | None beyond a leaked link (above) |
...
| Use cases | Specs use the real `PolicyAuthorizationService` with `principalWith()` (`test/support/principals.ts`). A TEACHER without standing is refused everything; an all-permission principal without standing cannot moderate; no live use case passes `ownerUserId`; no route issues a token without the `community.live.join` permit. P5.1: for owners, members, delegates and overseers, OPEN and LOCKED, each operation `me.operations` lists is exactly what its route then does (`me-operations.spec.ts`) | P2, P5.1, P6 |
...
| Flutter | Repository parsing against `MockClient` (unknown enums → unknown, missing booleans → false); capability-driven UI (every action shown iff its server boolean is true; no role reads); frame parsing from the golden fixtures, version ≠ 1 dropped; `LiveSessionController` state machine with fakes; `CommunityController` version handling; the `/invite` flow never logs the token; mock parity; architecture tests; the layout table at every viewport; attendance screens never compute a ratio. P5's part landed without the `/invite` flow ([realtime.md §C8](realtime.md#c8-tested)); P5.1's added it and the management flows: the invite flow (a link at start and while running, signed out then in, malformed, refused for good, no connection then an explicit retry, one request per tap), the pure link functions on the VM, the management controllers (one request per target, reads after each answer, no read sent before a change shown after it) and screens (every action from `me` alone), and mock parity. Beyond the suites, two one-off checks, not committed: the app's `HttpCommunityRepository` driven against a running backend on PostgreSQL through every P5.1 call and refusal (passed), and a Playwright run (Chromium) of the release web build against it, 17 of 17 checks passed — the token scrubbed at a cold start and nowhere in the URL, `history.state`, storage or the title; sign-in; exactly one join request, the token in its body only; the community opened; back on `/invite`, nothing shown and nothing sent; a revoked link refused; a link opened while running captured and joined; a malformed one sending nothing | P0, P5 (landed), P5.1 (landed), P7, P9 |
...
| P5 Community realtime and Flutter communities | **Landed 2026-09-24.** `ConnectionManager.onlineUserIds()` and `onlineAudience` (`realtime/application/online-audience.ts`: no call when nobody is online, at most 1 + ⌈A/1000⌉ page calls); G1: `MessagingRealtimeRelay` resolves its recipients through it, and the existing relay specs pass unmodified; `CommunitiesRealtimeRelay`, wired by `RealtimeModule` importing `CommunitiesModule` and reaching only its contracts (`realtime-boundaries.spec.ts`), with every audience re-asked of Communities at delivery time (the person's latest stint; a lock's `heads`, then the ACTIVE members online) and narrowed by `COMMUNITY_VIEW_CEILING`, P5's one Communities contract addition, which `community.view`'s act rule now reads; the five `community.*` frames of protocol v1 and their golden fixtures in `backend/test/fixtures/realtime-frames/`; no client frame, `subscribe` still conversation-only, the version still 1; relay, WebSocket API, Postgres and scale specs, with `EXPLAIN` at 30,000 ACTIVE and 100,000 departed stints. Flutter: a read-only `CommunityRepository` (HTTP and mock), the models, the `CommunityEvent` family, `/communities`, `/communities/:communityId` and `…/members` driven by `me.capabilities` and `me.participation` (never roles), `conversationForCommunity` and `Conversation.communityId` with the chat opened as an ordinary conversation, and `community_boundaries_test.dart`. No migration, domain event, notification type or messaging protocol change; posting stays switched off above 250 members until G3 (P8) and G4 hold. **Narrowed on purpose:** the `/invite#<token>` deep link and every management action in the app (adding and removing members, leaving, creating and revoking invitations, joining by token, locking and unlocking, grants and their revocation, ownership transfer) are not in P5; P5.1 built them (the next row), except adding members by id and mobile app links, which stay deferred. **Changed from the plan:** the `community.access.changed` `eventId` ends in a digest of the fact instead of its time, so two access changes within one millisecond never share an id ([§16.2](#162-frames-added-to-protocol-v1)). Choices made during implementation are recorded in [realtime.md Part C](realtime.md#part-c--communities-in-real-time) |
| P5.1 Community management and the invitation link (Flutter) | **Landed 2026-09-26.** Four decisions, taken by the user on 2026-09-26 at the review of P5.1's stop conditions: (1) **the server reports the operations that are not acts**, so the app never works one out from standing or roles; (2) **members are added through invitation links only**: the direct add (`POST /communities/:communityId/members`) gets no app UI, because no policy says how a manager may find the people to add (Q6, Q22, Q31, Q50); (3) **invitation links are the web app's**: it writes `<its base href>invite#<token>` and redeems links, native builds write none, and mobile app links are not configured; (4) **joining asks first**: an explicit Join tap sends one POST. Backend: `COMMUNITY_OPERATIONS` in `communities/contracts/capabilities.ts` (`community.invitations.manage`, `community.grants.manage`, `community.ownership.transfer`, `community.leave`), P5.1's one Communities contract addition, reported additively as `CommunityResponse.me.operations`; `me()` (`community-authorization.service.ts:235-270`) decides each by the rule its route uses — `LINK_MANAGEMENT_RULE`, `decideOwnerOperation` over `MANAGE_GRANTS` and `TRANSFER_OWNERSHIP`, and leaving as the leave route asks it, through `mayLeave` (`domain/membership.ts`), now also the owner-cannot-leave check of both stores; `me-operations.spec.ts` checks, for owners, members, delegates and overseers, OPEN and LOCKED, that each listed operation is exactly what its route then does. No migration, domain event, route, frame or protocol change. Flutter: `CommunityRepository`'s management calls (HTTP and mock, the mock's copies of the server's rules labelled PROVISIONAL with their questions) and `ApiClient.put`; `/invite#<token>` captured in `main()` before `runApp`, scrubbed from the address bar and held in memory only; the invite screen (no invitation, sign in, Join; a refusal for good forgets the link, anything passing keeps it for an explicit retry, a malformed one sends nothing); on the community screen, invitation links, lock and unlock, and leave; on the roster, a member's capabilities (the server's grant list), make owner and remove, never on the viewer's own row; the invitations screen (a paged list, revoke; a new link shown once, on the web); each action shown only as `me` (capabilities, operations, status) allows; every write one request per target, never retried on its own, and read again once answered. Evidence beyond the suites: a one-off contract check of the app's HTTP repository against a running backend on PostgreSQL, and a Playwright run of the release web build (17 of 17 checks), neither committed ([§22](#22-testing-strategy)). Two older faults fixed on the way: the list's per-community refetch awaited itself forever (P5), and a refused token refresh threw a `CircularDependencyError` in debug builds instead of signing out. **Still deferred:** the direct add, until a people-lookup policy exists; mobile app links, and a public app address with which native builds could write links; choosing a link's terms in the app, and a link preview ([Q48](open-questions.md#q48--invitation-links)). Choices made during implementation are recorded in [communities.md](communities.md) and [realtime.md §C6](realtime.md#c6-the-clients-contract-for-community-frames) |
...
| **P5** Community realtime and Flutter communities | `onlineUserIds`, `OnlineAudience` (messaging relay swaps onto it: G1); `CommunitiesRealtimeRelay`; `community.*` frames; golden fixtures; Flutter `CommunityRepository`, frame families, capability-driven screens, `/invite`, `conversationForCommunity`. As landed: the repository reads only; `/invite` and the app's management actions came in P5.1 (the next row) | P2 (P3 for `access.changed`) | `OnlineAudience` property test; relay and WebSocket API tests; existing relay specs; Flutter parsing, golden, parity, boundary and layout tests | P2 |
| **P5.1** Community management and the invitation link (Flutter; added after P5) | `me.operations` on the server; the repository's management calls, all but the direct add; `/invite#<token>` on the web; the management screens and the invite screen, each action from `me` alone. Not in it: the direct add, mobile app links, link terms or a preview in the app (the status table above) | P5; the four decisions of the status table | `me-operations.spec.ts`; Flutter model, repository (`MockClient`), mock parity, controller, screen, invite-flow, link, navigation, layout and boundary tests; a one-off contract check against a running backend and a Playwright run of the web build | P5 |
