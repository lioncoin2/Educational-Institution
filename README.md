...
| المكوّن | الحالة |
|---|---|
| **تطبيق Flutter** (`app/`) | ✅ يعمل تجريبياً بلا خادم (بيانات معلَّمة)، أو مع الخادم عبر `API_BASE_URL`: الدخول والرسائل والإشعارات والبيانات الأكاديمية والمجتمعات وإدارتها حقيقية، ورابط الدعوة `/invite` يعمل في نسخة الويب |
...
| **المجتمعات** (Communities، المراحل P2–P5.1) | ✅ المجتمعات والعضوية وروابط الدعوة والقفل وتفويض الصلاحيات، ومحادثة المجتمع داخل المراسلة، وتنبيهات فورية بالمعرِّفات فقط لمن يعنيهم الأمر عبر الاتصال المباشر، وشاشات Flutter لعرض المجتمع وإدارته (روابط الدعوة، القفل وفكّه، المغادرة، صلاحيات العضو، نقل الملكية، الإزالة) بحسب ما يتيحه الخادم وحده (`me`)، ورابط الدعوة `/invite` في نسخة الويب · ⏸ إضافة الأعضاء مباشرةً بحساباتهم مؤجَّلة حتى تُقرَّر سياسة للبحث عن الأشخاص (الانضمام بروابط الدعوة وحدها)، وروابط تطبيقات الجوّال مؤجَّلة، والجلسات الصوتية المرتبطة بالمجتمع مقترحة (P6–P7)، والحضور مُعلَّق |
...
| [`docs/architecture/communities-live-attendance.md`](docs/architecture/communities-live-attendance.md) | **مُعتمَد؛ نُفِّذت المراحل P0–P5.1** (الحواجز المعمارية، تقوية `live`، نواة المجتمعات وتفويضها، محادثة المجتمع، الاتصال المباشر للمجتمعات وشاشاتها في Flutter، وإدارتها من التطبيق ورابط الدعوة)، **والباقي مقترح** (P6–P8 وP10–P12)، **والحضور (P9) مُعلَّق.** المستند المحوري لتصميم المجتمعات والجلسات الصوتية المباشرة ولقطات الحضور: الحكم، الملكية، مخطّط الاعتماديات، العقود، الأحداث، التهديدات، التوسّع، مراحل التنفيذ. «المجتمع» (Community) هو ما يسمّيه الموجز «Group»؛ ولم يُسمَّ «مجموعة» لأنّ `GROUP` نوعُ محادثةٍ في المراسلة، و«مجموعة» كلمةٌ يستعملها قسم التهجي، ومعناها سؤالٌ مفتوح (Q36) |
| [`docs/architecture/communities.md`](docs/architecture/communities.md) | **مُنفَّذ** (P2 النواة، P3 التفويض؛ وثابتٌ واحد في العقد لكلٍّ من P4 وP5 وP5.1، وثابت P5.1 هو ما يُبلِّغ عنه `me.operations`). وحدة `communities`: المجتمع، العضوية، روابط الدعوة، القفل، تفويض الصلاحيات |
