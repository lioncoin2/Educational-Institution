...
| المكوّن | الحالة |
|---|---|
| **تطبيق Flutter** (`app/`) | ✅ يعمل تجريبياً بلا خادم (بيانات معلَّمة)، أو مع الخادم عبر `API_BASE_URL`: الدخول والرسائل والإشعارات والبيانات الأكاديمية والمجتمعات (قراءةً فقط) حقيقية |
...
| **المجتمعات** (Communities، المراحل P2–P5) | ✅ المجتمعات والعضوية وروابط الدعوة والقفل وتفويض الصلاحيات، ومحادثة المجتمع داخل المراسلة، وتنبيهات فورية بالمعرِّفات فقط لمن يعنيهم الأمر عبر الاتصال المباشر، وشاشات Flutter للقراءة فقط · ⏸ إدارة المجتمع من التطبيق ورابط `/invite` مؤجَّلان، والجلسات الصوتية المرتبطة بالمجتمع مقترحة (P6–P7)، والحضور مُعلَّق |
| الوحدات المكتملة في الـ Backend | `identity` · `live` (الغرف الصوتية) · `files` · `messaging` · `realtime` · `notifications` · `academic` · `communities` |
...
| [`docs/architecture/communities-live-attendance.md`](docs/architecture/communities-live-attendance.md) | **مُعتمَد؛ نُفِّذت المراحل P0–P5** (الحواجز المعمارية، تقوية `live`، نواة المجتمعات وتفويضها، محادثة المجتمع، الاتصال المباشر للمجتمعات وشاشاتها في Flutter)، **والباقي مقترح** (P6–P8 وP10–P12)، **والحضور (P9) مُعلَّق.** المستند المحوري لتصميم المجتمعات والجلسات الصوتية المباشرة ولقطات الحضور: الحكم، الملكية، مخطّط الاعتماديات، العقود، الأحداث، التهديدات، التوسّع، مراحل التنفيذ. «المجتمع» (Community) هو ما يسمّيه الموجز «Group»؛ ولم يُسمَّ «مجموعة» لأنّ `GROUP` نوعُ محادثةٍ في المراسلة، و«مجموعة» كلمةٌ يستعملها قسم التهجي، ومعناها سؤالٌ مفتوح (Q36) |
| [`docs/architecture/communities.md`](docs/architecture/communities.md) | **مُنفَّذ** (P2 النواة، P3 التفويض؛ وثابتٌ واحد في العقد لكلٍّ من P4 وP5). وحدة `communities`: المجتمع، العضوية، روابط الدعوة، القفل، تفويض الصلاحيات |
| [`docs/architecture/community-chat.md`](docs/architecture/community-chat.md) | **مُنفَّذ** (P4؛ وجانب التطبيق وبوابة G1 في P5). محادثة المجتمع داخل وحدة المراسلة: إسقاطٌ مُسمّى لعضوية المجتمع، لا نظام رسائل ثانٍ |
| [`docs/architecture/live.md`](docs/architecture/live.md) | **نُفِّذت منه P1 وحدها** (مع حواجز P0): تقوية الوحدة القائمة، وهي ما تزال مرتبطة بالحلقة وفي الذاكرة — صلاحيات كاملة، منافذ ضيّقة لـ LiveKit، رفع يد وقرارات متكرّرة بلا أثر مضاعف، رمز دخول 120 ثانية. **والباقي مقترح:** الجلسات المرتبطة بالمجتمع ومشاركة الشاشة وسعة الجلسة (P6)، والاتصال المباشر وFlutter (P7)، وربط الوسائط (P7b). تطوّر وحدة `live`: جلسات مرتبطة بالمجتمع، رفع اليد والمتحدّثون، مشاركة الشاشة، منافذ LiveKit، سعة الجلسة |
| [`docs/architecture/attendance.md`](docs/architecture/attendance.md) | **مُعتمَد تصميماً فقط، والتنفيذ مُعلَّق** (P9؛ لم يُبنَ منه شيء). لقطات الحضور أثناء الجلسة المباشرة: من رُصد في الجلسة لحظة الضغط على الزر؛ أمّا معنى «حاضر» فسؤال مفتوح (Q68) |
